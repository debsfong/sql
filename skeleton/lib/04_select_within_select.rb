...
def population_constraint
  ...
  execute(<<-SQL)
    ...
    WHERE
      population BETWEEN
        (SELECT population + 1 FROM countries WHERE name = 'Canada')
        AND
...
def sparse_continents
  ...
  # Find continents where NO countries have populations > 25,000,000
  # Name all countries of those continents
